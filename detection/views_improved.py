...
def process_image_and_predict(test_result):
    ...
    try:
        ...
        # Generate explainability images if Captum is available
        try:
            from .explainability import generate_explainability_images_onnx
            generate_explainability_images_onnx(image_path, test_result)
        except ImportError:
            logger.warning("Captum not available, skipping explainability generation")
        except Exception as e:
